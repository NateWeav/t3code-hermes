...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "T3 Hermes");
  expect(findCaptureDestination([window(99, "T3 Hermes"), target], 42, "T3 Hermes")).toBe(target);
  expect(findCaptureDestination([window(99, "T3 Hermes")], 42, "T3 Hermes")).toBeUndefined();
