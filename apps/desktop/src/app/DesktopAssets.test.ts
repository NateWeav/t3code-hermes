  ...
  it.effect("uses canonical source-tree icons for unpackaged development", () =>
    Effect.gen(function* () {
      ...
      const fileSystemLayer = FileSystem.layerNoop({
        exists: (path) => Effect.succeed(String(path).includes("/assets/hermes/")),
      });
      ...
      assert.match(Option.getOrThrow(icons.ico), /assets\/hermes\/icon\.ico$/);
      assert.match(Option.getOrThrow(icons.png), /assets\/hermes\/icon-1024\.png$/);
