...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/T3 Hermes.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/T3 Hermes.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("isolates Hermes from an upstream install and its ambient configuration", () =>
    Effect.gen(function* () {
  ...
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.appUserModelId, "com.nateweav.t3hermes.dev");
      assert.equal(environment.linuxWmClass, "t3-hermes-dev");
      assert.equal(environment.linuxDesktopEntryName, "com.nateweav.T3Hermes.Development.desktop");
      assert.deepEqual(
  ...
  it.effect("uses the stable desktop entry as the packaged Linux portal identity", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.linuxDesktopEntryName, "com.nateweav.T3Hermes.desktop");
    }),
  );
  ...
  it.effect("keeps implicit development state separate from production state", () =>
    Effect.gen(function* () {
      ...
      assert.equal(development.stateDir, "/Users/alice/.t3-hermes/dev");
      assert.equal(production.stateDir, "/Users/alice/.t3-hermes/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " com.nateweav.t3hermes.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.nateweav.t3hermes.dev.local");
    }),
  );
