...
const MAC_ACCESSIBILITY_PERMISSION_MESSAGE =
  "Allow Accessibility in System Settings, then restart T3 Hermes.";
const MAC_BOTH_PERMISSIONS_MESSAGE =
  "Allow Accessibility and Screen Recording in System Settings, then restart T3 Hermes.";
...
export class DesktopSnapShot extends Context.Service<
  DesktopSnapShot,
  {
    ...
    /** Capture the foreground window in place, including T3 Hermes itself. */
...
export const make = Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
      ...
      if (snapshot.linuxActivationFailure) {
        yield* Effect.logWarning(
          "The compositor could not activate T3 Hermes after the snapshot",
  ...
  const checkShortcut = Effect.fn("desktop.snapShot.checkShortcut")(function* (
    ...
    if (mode === "portal" && niriSocketPath()) {
      return {
        available: false,
        message: "Configure the capture shortcut in your Niri config, not in T3 Hermes.",
      };
    }
    ...
    if (mode === "portal" && niriSocketPath()) {
      const registered = yield* Effect.tryPromise(async () => {
        ...
        return startNiriCaptureShortcut(linuxAppId, onCurrentShortcut, () => {
          void runPromise(
            setShortcutFailure("The Niri capture endpoint disconnected. Restart T3 Hermes."),
          ).catch(() => undefined);
        });
      }).pipe(
        Effect.tap((stop) =>
      ...
      yield* Ref.set(stateRef, {
        ...
        shortcutMessage: registered
          ...
          : "Could not start the Niri capture endpoint. Another T3 Hermes instance may be using it.",
        message: null,
      });
      return;
    }
