...
it.each([true, false])("uses the active config syntax and user bindings (Lua: %s)", async (lua) => {
  ...
  const result = await hyprlandCaptureShortcut("com.nateweav.T3Hermes", directory);
  expect(result.shortcutConfigPath).toBe(
    NodePath.join(config, lua ? "bindings.lua" : "bindings.conf"),
  );
  expect(result.shortcutBinding).toBe(
    lua
      ? 'hl.bind("CTRL + SHIFT + 2", hl.dsp.global("com.nateweav.T3Hermes:capture-window"))'
      : "bind = CTRL SHIFT, 2, global, com.nateweav.T3Hermes:capture-window",
  );
