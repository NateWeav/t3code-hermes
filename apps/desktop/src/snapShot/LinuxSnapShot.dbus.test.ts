...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.T3HermesSnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    expect(await portal.backend("com.nateweav.T3Hermes")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.nateweav.T3Hermes")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.nateweav.T3Hermes")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    const owner = await server.call(
      new Message({
        ...
        member: "GetNameOwner",
        signature: "s",
        body: ["com.nateweav.T3Hermes.SnapShot"],
      }),
    );
    ...
    expect(await updated.backend("com.nateweav.T3Hermes")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.nateweav.T3Hermes", {
      flash: true,
      animate: true,
    });
    expect(snapshot.feedback?.animationStarted).toBe(true);
    await snapshot.feedback!.activate("T3 Hermes");
    ...
    expect(activateTitle).toBe("T3 Hermes");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("com.nateweav.T3Hermes")).toBe("kde");
    ...
    stopNiriShortcut = await startNiriCaptureShortcut(
      "com.nateweav.T3Hermes.NiriTest",
      triggered,
      failed,
      niriBus,
    );
    const command = [
      ...
      "--session",
      "--dest",
      "com.nateweav.T3Hermes.NiriTest.SnapShot",
      "--object-path",
    ...
    const invalid = server.call(
      new Message({
        destination: "com.nateweav.T3Hermes.NiriTest.SnapShot",
