...
it("selects the native adapter without needing a portal or GNOME extension", async () => {
  expect(await getLinuxCaptureSupport("com.nateweav.T3Hermes")).toEqual({
    linuxBackend: "niri",
    linuxFeedbackAvailable: false,
  });
  const snapshot = await captureLinuxWindow("com.nateweav.T3Hermes");
...
it("does not activate T3 until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "T3 Hermes" },
    { ...window, id: 2, pid: process.pid, title: "Other T3" },
    { ...window, id: 3, pid: process.pid, title: "T3 Hermes" },
  ];
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: {
          window: { ...window, id: 4, pid: process.pid, title: "T3 Hermes" },
        },
      });
  };
  await snapshot.feedback!.activate("T3 Hermes");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "T3 Hermes" }));
  await expect(snapshot.feedback!.activate("T3 Hermes")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("T3 Hermes")).rejects.toThrow("cancelled");
...
it("rejects compositor errors and cleans up its temporary image", async () => {
  ...
  await expect(captureLinuxWindow("com.nateweav.T3Hermes")).rejects.toThrow("window disappeared");
  expect(await NodeFSP.stat(NodePath.dirname(capturePath!)).catch(() => undefined)).toBeUndefined();
});
