...
import { useHermesProviderPresent } from "../hermes/HermesRouteScreen";
  ...
  const handleDeviceNotificationsChange = useCallback(
    (enabled: boolean) => {
      ...
      Alert.alert(
        ...
        "Notification permission is controlled by iOS. Open Settings to disable notifications for T3 Hermes.",
        [
          { text: "Cancel", style: "cancel" },
          { text: "Open Settings", onPress: () => void Linking.openSettings() },
        ],
      ...
      >
        <View className="gap-3">
          ...
          <Text className="px-2 text-sm text-foreground-muted">
            T3 Hermes works locally without signing in. Cloud features are optional.
          </Text>
        </View>
...
function GeneralSettingsSection() {
  const hermesProviderPresent = useHermesProviderPresent();
  ...
  return (
    <SettingsSection title="General">
      ...
      {hermesProviderPresent ? (
        <SettingsRow icon="clock" label="Hermes" target="SettingsHermes" />
      ) : null}
    </SettingsSection>
  );
}
