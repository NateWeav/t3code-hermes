...
export class DesktopAppSshUnsupportedError extends Schema.TaggedError<DesktopAppSshUnsupportedError>()(
  ...
  override get message(): string {
    return "`t3-hermes app` only controls a desktop app on the same machine. It cannot run over SSH.";
  }
}
...
export class DesktopAppPlatformUnsupportedError extends Schema.TaggedError<DesktopAppPlatformUnsupportedError>()(
  ...
  override get message(): string {
    return `\`t3-hermes app\` is not supported on ${this.platform}.`;
  }
}
...
export class DesktopAppUnreachableError extends Schema.TaggedError<DesktopAppUnreachableError>()(
  ...
  override get message(): string {
    return "Could not reach the T3 Code desktop app. Start or update the desktop app on this machine, then run `t3-hermes app` again. A running T3 Code server is not enough.";
  }
}
...
const appEnvironment = Config.all({
  t3Home: Config.string("T3HERMES_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
