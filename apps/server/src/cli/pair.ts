/**
 * `t3-hermes pair` - mint a pairing token for an already-running server and print it
...
export class NoRunningServerError extends Schema.TaggedError<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      ...
      "Start one with `npx t3-hermes serve`, or connect this machine with T3 Connect: `npx t3-hermes connect`.",
    ].join("\n");
  }
}
...
const probeEnvironmentDescriptor = (
  ...
  Effect.gen(function* () {
    ...
    // it as unreachable lets `t3-hermes pair --tailscale` repair its own mapping
...
const discoverPairTarget = Effect.fn("pair.discoverPairTarget")(function* (
  ...
  if (explicitBaseDir !== undefined && explicitBaseDir.trim().length > 0) {
    ...
    // outranks the shared home, so `t3-hermes pair` in a worktree pairs with the dev
    ...
    const envHome = yield* Config.string("T3HERMES_HOME").pipe(Config.option);
