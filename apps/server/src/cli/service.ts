...
export function formatServiceStatus(
  ...
  if (!status.installed) {
    return "T3 Code service\n  Status: not installed\n  Next: Run `t3-hermes service install`.";
  }
  ...
  if (
    ...
    return [
      "T3 Code service",
      `  Status: installed · t3-hermes@${installedVersion} (newer than this t3-hermes@${cliVersion} CLI)`,
      `  Unit: ${status.unitPath}`,
      `  Logs: ${status.logPath}`,
      ...problems,
      `  Next: Use \`npx t3-hermes@${installedVersion} service update\` to repair it, or pass \`--allow-downgrade\` explicitly.`,
    ].join("\n");
  }
  return [
    "T3 Code service",
    `  Status: ${status.current ? `installed · t3-hermes@${installedVersion}` : "needs an update or repair"}`,
    `  Unit: ${status.unitPath}`,
    `  Logs: ${status.logPath}`,
    ...problems,
    ...(status.current ? [] : [`  Next: Run \`npx t3-hermes@${cliVersion} service update\`.`]),
  ].join("\n");
}
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(
            `T3 Code service is already installed with t3-hermes@${packageJson.version}.`,
          );
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} T3 Code service with t3-hermes@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
...
const serviceUpdateCommand = Command.make("update", serviceReconcileFlags).pipe(
  Command.withDescription(
    "Update or repair the background service using this CLI version. Use `npx t3-hermes@latest service update` for the latest release.",
  ),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        const result = yield* reconcileService({ allowDowngrade: flags.allowDowngrade });
        if (!result.changed) {
          yield* Console.log(`T3 Code service is already using t3-hermes@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} T3 Code service with t3-hermes@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (
    ...
    yield* Console.log(
      `A newer t3-hermes@${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
