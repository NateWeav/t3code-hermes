...
/**
 * `t3-hermes theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `t3-hermes theme set`.
 *
...
export class ThemeTargetMissingError extends Schema.TaggedError<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `t3-hermes theme clear` to remove the theme.";
  }
}

const envT3Home = Config.string("T3HERMES_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then T3HERMES_HOME,
  // then the default home. A provisioning script exporting T3HERMES_HOME must
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `t3-hermes theme set ocean` publish ./ocean whenever the cwd happens to
