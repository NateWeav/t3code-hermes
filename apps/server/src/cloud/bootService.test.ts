...
const linuxRuntime = "/home/theo/.t3-hermes/runtime/versions/1.2.3/t3";
const linuxPlan = {
  program: [linuxRuntime, "__service-launcher"],
  baseDir: "/home/theo/.t3",
  logPath: "/home/theo/.t3-hermes/userdata/logs/boot-service.log",
  unitPath: "/home/theo/.config/systemd/user/t3-hermes.service",
};
...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.nateweav.t3hermes.service.plist",
};
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable t3-hermes.service")
        control.enabled = true;
      if (!failed && command === "systemctl --user restart t3-hermes.service")
        control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop t3-hermes.service" ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop t3-hermes.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      const { service, fs, statePath, commands, timeouts, runtime } = yield* makeHarness();
      const path = yield* Path.Path;
      const before = yield* service.status;
      const upstreamUnit = path.join(path.dirname(before.unitPath), "t3code.service");
      yield* fs.makeDirectory(path.dirname(upstreamUnit), { recursive: true });
      yield* fs.writeFileString(upstreamUnit, "upstream service");
      const plan = yield* service.install();
      ...
      expect(yield* fs.readFileString(upstreamUnit)).toBe("upstream service");
      ...
      expect(timeouts.get("systemctl --user disable --now t3-hermes.service")).toEqual(
      ...
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ...
            : [
                "launchctl bootout --wait gui/501/com.nateweav.t3hermes.service",
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.nateweav.t3hermes.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(
        timeouts.get("launchctl bootout --wait gui/501/com.nateweav.t3hermes.service"),
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.nateweav.t3hermes.service",
        "launchctl enable gui/501/com.nateweav.t3hermes.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.nateweav.t3hermes.service";
