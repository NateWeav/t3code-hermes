...
/**
 * A pinned runtime is an exact `t3-hermes@<version>` npm-installed into
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "t3-hermes", "dist", "bin.mjs"),
...
/**
 * Installs `t3-hermes@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "t3-hermes", "dist", "bin.mjs"),
