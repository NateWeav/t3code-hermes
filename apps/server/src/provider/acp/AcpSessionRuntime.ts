...
export class AcpSessionRuntime extends Context.Service<
  AcpSessionRuntime,
  {
    ...
    /** Slash commands advertised by the agent, when it has advertised any. */
    readonly getSlashCommands: Effect.Effect<ReadonlyArray<ServerProviderSlashCommand> | undefined>;
    /** Resolves with the first slash-command advertisement from the agent. */
...
>()("t3-hermes/provider/acp/AcpSessionRuntime") {}
  ...
  Effect.gen(function* () {
    ...
    for (const event of parsed.events) {
      if (event._tag === "AvailableCommandsUpdated") {
        const commands = parseAcpAvailableCommands(event.availableCommands);
        yield* Ref.set(slashCommandsRef, commands);
        // Later advertisements update the ref; only the first completes this receipt.
        yield* Deferred.succeed(slashCommandsDeferred, commands);
