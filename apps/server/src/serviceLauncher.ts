...
// `t3-hermes service update`. Keep runtime imports limited to Node built-ins.
...
const runtimePaths = (baseDir: string, version: string) => {
  ...
  return {
    versionDir,
    entryPath: NodePath.join(versionDir, "node_modules", "t3-hermes", "dist", "bin.mjs"),
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected t3-hermes@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
...
async function main(): Promise<void> {
  const baseDir = process.env.T3HERMES_HOME?.trim();
  if (baseDir === undefined || baseDir === "") {
    throw new Error("T3HERMES_HOME is required by the T3 Code service launcher.");
  }
