...
export const make = Effect.gen(function* () {
  ...
  const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(function* (
    ...
    dirs.push(
      { provider: "hermes", dir: hermesHome, sourcePath: path.join(hermesHome, "state.db") },
      { provider: "opencode", dir: path.dirname(openCodeDb), sourcePath: openCodeDb },
    );
    return dirs;
  });
  ...
  /** One provider source's read, before rates are involved. */
  interface ScannedDir {
    ...
    /**
     * Canonical database records for Hermes/OpenCode. `null` means the source
     * existed but could not be read; `undefined` identifies transcript sources.
     */
    readonly directRecords?: readonly UsageRecord[] | null;
    /** Parsed records per transcript, or `null` when the source does not exist. */
    readonly files:
  ...
  const collectDirs = Effect.fn("UsageService.collectDirs")(function* (
    ...
    for (const { provider, dir, sourcePath = dir, fileName } of dirs) {
      const volumeId = yield* Effect.promise(() => readDirectoryVolumeId(dir));
      const exists = yield* fileSystem
        .exists(sourcePath)
        .pipe(Effect.catchCause(() => Effect.succeed(false)));
      if (!exists) {
        scanned.push({ provider, dir, sourcePath, volumeId, files: null });
        continue;
      }

      if (provider === "hermes") {
        const directRecords = yield* Effect.sync(() =>
          readHermesUsageRecords(sourcePath, windowStartMs),
        );
        scanned.push({ provider, dir, sourcePath, volumeId, directRecords, files: [] });
        continue;
      }
      ...
      scanned.push({ provider, dir, sourcePath, volumeId, files: parsedFiles });
    }
    return scanned;
  });
  ...
  const scanSummary = Effect.fn("UsageService.scanSummary")(function* (
    ...
    for (const { provider, dir, sourcePath, volumeId, files, directRecords } of scannedDirs) {
      if (files === null) {
        sources.push({
      ...
      if (directRecords !== undefined) {
        if (directRecords === null) {
          sources.push({
            ...
            message:
              provider === "hermes"
                ? "Hermes state database could not be read."
