...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
      ...
      try {
        ...
        await Promise.all(
          composerImages.map(async (image) => {
            try {
              // Keep generic draft binaries bounded so localStorage remains usable.
              if (image.type === "file" && image.sizeBytes > 1_000_000) return;
  ...
  const addComposerImages = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      // Images go down the compression path, so they are held to the formats
      // the encoder can actually re-emit. Everything else rides along as a
      // generic file attachment and only has to fit the upload cap.
      if (file.type.startsWith("image/")) {
        ...
        error = `'${file.name}' exceeds the ${Math.floor(PROVIDER_SEND_TURN_MAX_FILE_BYTES / (1024 * 1024))} MiB file limit.`;
        continue;
      }
                  ...
                  {composerImages
                    ...
                    .map((image) =>
                      image.type === "file" ? (
                        <div
                          key={image.id}
                          className="relative flex h-12 max-w-72 items-center gap-2 rounded-lg border border-border/80 bg-background px-3 pr-9"
                        >
            ...
            >
              <div className="-m-1 -ms-3.5 flex min-w-0 flex-1 items-center gap-1 overflow-x-auto p-1 ps-3.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                ...
                <Button
                  ...
                  variant="ghost"
                  size="icon-sm"
                  // Leading control in a scroll container that pads its start
                  // edge so focus rings are not clipped; the pull-back keeps it
                  // optically flush with the composer text above.
                  className="-ms-2.5 shrink-0"
