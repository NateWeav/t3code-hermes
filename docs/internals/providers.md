...
The Hermes fork adds the `hermes` driver alongside the upstream built-ins. It runs `hermes acp`
through the shared ACP session runtime, supports multiple configured instances, and reads credentials
from Hermes' own `HERMES_HOME` (normally `~/.hermes`). T3 Code probes and launches it but deliberately
does not install or update the Python environment. Keep Hermes-specific model, mode, compaction, and
slash-command translation at the [Hermes ACP boundary](../../apps/server/src/provider/acp/HermesAcpSupport.ts)
rather than adding Hermes branches to orchestration.
