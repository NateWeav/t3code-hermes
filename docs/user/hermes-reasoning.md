...
that model actually accepts — from `minimal` up to `ultra`, plus the option to turn thinking off.
...
model is required to think and cannot be turned off, the "off" option is left out.
