...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // Whether the status ring around a sidebar card's project icon moves
  // (turning while working, breathing while waiting). Off leaves the rings
  // in place and still, exactly as `prefers-reduced-motion` renders them.
  sidebarStatusRingMotion: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
...
export const ClientSettingsPatch = Schema.Struct({
  ...
  sidebarStatusRingMotion: Schema.optionalKey(Schema.Boolean),
